...
public:
    ...
    GridTest(const Properties &props) : Base(props) {
        ...
        {
            m_dataCopyC = drjit::empty<FloatArrayC>(m_floatSize);
            memcpy(m_dataCopyC.data(), m_pGridData32, m_byteSize);
        }else if (jit_has_backend(JitBackend::LLVM))
        {
            m_dataCopyL = drjit::empty<FloatArrayL>(m_floatSize);
            memcpy(m_dataCopyL.data(), m_pGridData32, m_byteSize);
        }else{
            //TODO: debug this memcopy?
